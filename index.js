  ...
  if (product.sale != 0) {
    productItemWrapper = `
                <div class="productItemWrapper ${size}">
                  <a href="#" class="productItem" data-id="${product.id}" onclick="getDetailProduct(this)">
                      <img src="${product.imageUrl}" alt="${product.name}" class="productItem__img">
...
async function fetchDataByType(type, listProduct) {
  try {
    ...
    if (Array.isArray(data)) {
      data.forEach(item => {
        const productDiv = createProductItem(item, "l-2-4");
...
function showProductByType(item) {
  let _url;
  if (item.value !== "all") {
    _url = `http://localhost:8080/plant/find/?types=${item.value}&maxPrice=0`;
    localStorage.setItem('type', item.value);
    window.location.href = 'listProduct.html?types=' + item.value;
  } else {
    _url = `http://localhost:8080/plant/find/?types=&maxPrice=0`;
...
async function firstRedirectListProductPage() {
  await axios.get(localStorage.getItem('url'))
    .then(response => {
      ...
      if (Array.isArray(data)) {
        data.forEach(item => {
          const productDiv = createProductItem(item, "l-3");
  ...
  pagination.innerHTML = createPagination(localStorage.getItem('totalPages'), 1);
}
...
async function fetchDataInListPage(types, minPrice, maxPrice, page) {
  let func_url = 'http://localhost:8080/plant/find/?types=';
  axios.get(func_url + types.join(',') + '&minPrice=' + minPrice + '&maxPrice=' + maxPrice + '&pageNo=' + page)
    .then(response => {
      localStorage.setItem('totalPages', response.data.totalPages);
      localStorage.setItem('resultsListPage', JSON.stringify(response.data.data));
      window.history.pushState(null, "", 'listProduct.html?types=' + types.join(',') + '&minPrice=' + minPrice + '&maxPrice=' + maxPrice + '&page=' + page);
      showProductInListPage();
    })
    .catch(error => console.log(error));
  pagination.innerHTML = createPagination(localStorage.getItem('totalPages'), page);
}
...
function getIndexPagination(index) {
  ...
  fetchDataInListPage(types, minPrice, maxPrice, parseInt(index.value));

  pagination.innerHTML = createPagination(localStorage.getItem('totalPages'), parseInt(index.value));
}
    ...
    detailProduct =
      ...
      <div class="box-product">
          ...
          <div class="basic-info">
              ...
              <div class="price">${product.newPrice.toLocaleString()}đ <span>${product.oldPrice.toLocaleString()}đ</span></div>
              <div class="options">
...
function createElementCartPage(data) {
  ...
  for (let i = 0; i < cartItems.length; i++) {
    ...
    newRow.innerHTML = `
        ...
        <td><input type="number" value="${cartItems[i].quantity}"></td>
...
function openModal(item) {
  ...
  btnDangerModal.setAttribute("data-id", item.value);
}
